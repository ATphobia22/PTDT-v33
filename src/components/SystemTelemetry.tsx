...
const RealtimeChart: React.FC<{
  ...
  useEffect(() => {
    ...
    const x = d3.scaleTime().domain([t0, t1]).range([0, innerWidth]);
    ...
    g.append("g")
      ...
      .call((sel) =>
        sel.selectAll(".tick line").attr("stroke", "#334155").attr("stroke-dasharray", "2,2")
      );
  ...
  return (
    <div className="flex flex-col gap-4 font-sans text-slate-100">
      <div className="border-b border-slate-800 pb-3 mb-2 flex items-center justify-between">
        ...
        <span className="text-[10px] text-emerald-400 font-mono border border-emerald-400/30 bg-emerald-400/10 px-2 py-0.5 rounded">
          ILLUSTRATIVE
        </span>
      </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="bg-slate-950/50 border border-slate-800 rounded-lg p-3">
          <div className="flex justify-between items-center mb-2">
            <span className="text-[10px] font-mono text-slate-400 uppercase flex items-center gap-1">
              <Database size={12} /> Memory Allocation (GB)
            </span>
        ...
        <div className="bg-slate-950/50 border border-slate-800 rounded-lg p-3">
          <div className="flex justify-between items-center mb-2">
            <span className="text-[10px] font-mono text-slate-400 uppercase flex items-center gap-1">
              <Network size={12} /> Network I/O (Mbps)
            </span>
      ...
      <p className="text-[10px] text-slate-500 font-mono">
        Charts are synthetic browser metrics for UI continuity — not Prometheus host telemetry.
      </p>
    </div>
  );
}
