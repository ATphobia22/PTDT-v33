...
export const PARCELS = [
  {
    ...
    info: {
      ...
      isInundated: false,
      historicalNote:
        'Original 1820s settlement boundaries. Significant elevation delta limits standard flood exposure.',
  ...
  },
  {
    ...
    info: {
      ...
      isInundated: true,
      historicalNote:
        'German immigrant era land grant. Highly vulnerable lowland area near local stream channels.',
  ...
  },
  {
    ...
    info: {
      ...
      isInundated: false,
      historicalNote:
        'Wabash-Ohio confluence boundary property, functioning as community emergency point.',
...
const Terrain = ({
  ...
  const geometry = useMemo(() => {
    ...
    for (let i = 0; i < pos.count; i++) {
      ...
      const noise =
        Math.sin(x * 0.3) * Math.cos(z * 0.3) * 1.5 + Math.sin(x * 0.8 + z * 0.5) * 0.5;
  ...
  // Single material only — R3F mesh accepts one material child (or material array prop)
  return (
    <mesh
      ref={meshRef}
      geometry={geometry}
    ...
    >
      <meshStandardMaterial color="#374151" roughness={0.9} metalness={0.1} wireframe={false} />
    </mesh>
  );
};

const Water = ({ level = 5 }: { level: number; blockedPoints?: THREE.Vector3[] }) => {
  const meshRef = useRef<THREE.Mesh>(null);
  ...
  return (
    <mesh ref={meshRef} position={[0, level, 0]} receiveShadow rotation={[-Math.PI / 2, 0, 0]}>
  ...
  return (
    <group>
      {renderPoints.length > 1 && (
        <Line points={renderPoints} color="#f59e0b" lineWidth={15} dashed={false} />
      )}
      {renderPoints.map((p, i) => (
        <mesh key={i} position={p} castShadow>
          <sphereGeometry args={[0.5, 16, 16]} />
          <meshStandardMaterial
            color={i === renderPoints.length - 1 && draftPoint ? '#fbbf24' : '#d97706'}
          />
        </mesh>
      ))}
    </group>
...
export const WebGPU3DValley: React.FC<WebGPU3DValleyProps> = ({
  waterLevel = 2,
  onParcelClick,
}) => {
  ...
  return (
    <div className="w-full h-full relative bg-slate-900 rounded-xl overflow-hidden border border-slate-800">
      <Canvas shadows={{ type: THREE.PCFShadowMap }} camera={{ position: [0, 20, 40], fov: 45 }}>
        ...
        <Terrain onPointerDown={handleTerrainPointerDown} onPointerMove={handleTerrainPointerMove} />
      ...
      <div className="absolute top-4 right-4 flex flex-col gap-2 z-10 items-end">
        ...
        {savedBerms.length > 0 && !isDrawingMode && (
          <div className="bg-emerald-950/80 backdrop-blur border border-emerald-500/50 rounded-lg p-3 shadow-xl flex flex-col gap-2 mt-2 w-[240px] pointer-events-auto">
