  ...
  if (!isSystemOn) {
    return (
      <div className="w-screen h-screen bg-slate-950 flex flex-col items-center justify-center font-mono p-6 relative overflow-hidden select-none">
        <div className="absolute inset-0 z-0 bg-cover bg-center opacity-30" style={{ backgroundImage: 'url(/src/assets/images/ptdt_system_deactivated_1785575091081.jpg)' }} />
        ...
        <div className="max-w-md w-full bg-slate-900/40 border border-emerald-500/20 rounded-lg p-6 backdrop-blur-md shadow-2xl relative z-30 flex flex-col items-center gap-4 text-center">
          ...
          <button onClick={() => setSystemOn(true)} className="mt-2 w-full py-2 bg-emerald-500/20 hover:bg-emerald-500/30 border border-emerald-500/40 hover:border-emerald-500/60 text-emerald-400 rounded text-xs font-bold font-mono tracking-widest transition-all duration-300 cursor-pointer flex items-center justify-center gap-2">
            <Power size={14} /> BOOT PHYSICAL SYSTEM
          </button>
        </div>
      </div>
    );
