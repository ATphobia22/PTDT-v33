...
See also: `docs/API_KEY_MANAGEMENT.md`, `docs/MAPLIBRE_PMTILES.md`, `docs/EXTERNAL_WORKSTREAMS.md`.
...
|---------|---------------------|---------|------|--------|
| **USGS NWIS Instantaneous Values** | `https://waterservices.usgs.gov/nwis/iv/` | `src/integration/usgs_bridge.py` | None | Sites **03378500**, **03322000**; 00060 / 00065. Fallback if offline. |
| **USDA-NRCS Soil Data Access (SDA)** | `https://SDMDataAccess.sc.egov.usda.gov/Tabular/post.rest` | `src/api/federalProxies.ts` → `/api/nrcs-soil` | None | POST SQL; local soil fallback. |
| **OpenFEMA NFIP Claims** | `https://www.fema.gov/api/open/v2/FimaNfipClaims` | `src/api/federalProxies.ts` → `/api/openfema-claims` | None | OData filter; empty fallback. |
...
| PMTiles protocol | Offline/archive vector tiles (`docs/MAPLIBRE_PMTILES.md`) |
| Free style hosts | Default basemap without Mapbox/Cesium keys |
| Three.js / R3F | 3D visualization only |
...
| IDEM 319/205(j) instructions | https://www.in.gov/idem/nps/progress-evaluation/319205j-grant-application-instructions/ |
...
Homemade FEMA GO OAuth submit, auto-PE seals, invented BCR — see `docs/ANTI_FABRICATION.md`.
